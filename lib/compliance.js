/**
 * ComplianceJS Alpha Version 0.0.11
 * By s4m0k
 * 2014-04-04
 *
 */
...
(function() {
    ...
    var  initializeRules = function () {
        ...
        if (config.excludeDefaultRules === null || (typeof  config.excludeDefaultRules === "undefined")) {
            ...
            Logger.error ("ExcludeDefaultRules is " + typeof (config.excludeDefaultRules) + " value [" + config.excludeDefaultRules + "]");
    ...
    var showConsoleReport = function ( issues ) {
        ...
        for ( var index in issues ) {
            ...
            Logger.system ( issue.file + " @ line " + issue.line +", column " + issue.column);
            Logger.system  ("\t" + issue.message);
            Logger.system ("\t" + issue.code + "\n");
        }
    };
    ...
    var generateReport = function () {
        Logger.system ( "Generating report...");
        if ( config.reports && config.reports.length > 0) {
            ...
            Logger.system ("Length of reports : " + config.reports.length);
            // loop all the reports config
            for ( var index in config.reports ) {
                ...
                Logger.system ("Check if report folder (" + path.resolve (process.cwd(), repConf.dist ) + ") exists!");
                
                var absRepDist =  path.resolve (process.cwd(), repConf.dist );
                
                if ( fs.existsSync ( absRepDist )) {
                    
                    Logger.system ("Folder  " + absRepDist + " exists!  Deleting it ");
                    // delete it
                    removeFolder ( absRepDist );
                ...
                Logger.system ('Output root is  [' + outputRoot + ']');
                ...
                Logger.system ("Copying [" + issuesJS + "] to " + outputRoot + path.sep + "js" + path.sep + "issues.js");
                ...
                Logger.system ("__dirname : " + __dirname );
                
                var resPath = path.normalize ( __dirname + path.sep + ".." + path.sep + "templates" + path.sep +  "resources");
                var tmplPath = path.normalize ( __dirname + path.sep + ".." + path.sep + "templates" + path.sep +  repConf.template);
                
                ncp (resPath, outputRoot, function ( err ) {
                    if ( err ) {
                        Logger.error ( err );
                        throw err;
                    }
                    
                    ncp (tmplPath , outputRoot , function ( err ) {
                        if ( err ) {
                            Logger.error ( err );
                            throw err;
                        }
    ...
    var parseArguments = function  ( args, callback ) {
        ...
        Logger.system ("Parse argument callback!");
    ...
    exports.check  = function ( ) {
        ...
        try {
            ...
            parseArguments(process.argv, function ( err, args) {
                ...
                if ( (typeof args !== "undefined") &&  (args["--verbose"] === true) ) {
                    Logger.verbose = true;
                }
